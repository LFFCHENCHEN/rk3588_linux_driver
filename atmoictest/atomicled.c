...
{
    if (!atomic_dec_and_test(&platformled.lock))
    {
        atomic_inc(&platformled.lock);
        return -EBUSY;
    }
...
static int __init platformledInit(void)
{
    int ret;
    platformled.lock = (atomic_t)ATOMIC_INIT(0);
    atomic_set(&platformled.lock, 1);
...
static void __exit platformledExit(void)
{
