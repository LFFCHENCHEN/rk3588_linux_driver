...
static int led_open(struct inode *inode, struct file *filp)
{
    ...
    if (down_interruptible(&platformled.sem))
    {
        return -ERESTARTSYS; // Return error if semaphore is not available
    }
...
{
    int ret;
    sema_init(&platformled.sem, 1); // Initialize the semaphore
