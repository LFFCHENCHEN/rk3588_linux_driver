...
static int led_open(struct inode *inode, struct file *filp)
{
    ...
    if (platformled.isledbusy)
    {
        return -EBUSY;
    }
    spin_lock_irqsave(&platformled.lock, flags);
    platformled.isledbusy = 1;
    spin_unlock_irqrestore(&platformled.lock, flags);
...
{
    int ret;
    spin_lock_init(&platformled.lock);
    platformled.isledbusy = 0;
